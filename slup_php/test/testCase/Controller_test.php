	...
	class Controller_test extends Lf_testCase{
		...
		*	init,createDBDriver,getDB,setupSessionStart,getViewPath,getAction,getTitle,getAppMenu
		...
		public function create(){
			...
			//init
				...
				if(AppConfig::isSsl()){
					...
					//リダイレクトのデータを挿入
					$redirectCode=AppConfig::createRedirectCode($this->app->getDB(ControllerRunnable::basicDbIndex));	
					//リダイレクトコードなし	
					$this->getControl()->equalsTrue(count(($data=Redirector::find($this->app->getDB(ControllerRunnable::basicDbIndex),
						Redirector::createModel(array(Redirector::id=>$redirectCode))))>0));//データが消えていない
					//リダイレクトコード有り
					$_GET[AppConfig::getRedirectIndex()]=$redirectCode;
					$this->app->setupNotSsl_test();
					$this->getControl()->equalsTrue(count(($data=Redirector::find($this->app->getDB(ControllerRunnable::basicDbIndex),
						Redirector::createModel(array(Redirector::id=>$redirectCode))))===0));//データが消えている
		...
		public function testRegisterSessionUser(){
			...
			$this->app->setUser_test(Sl_user::createModel(array(Sl_user::id=>"-1",Sl_user::password=>TestUser::testPasswordValue)));
			$this->getControl()->equals(false,$this->app->registerSessionUser_test());
			$this->app->setupSessionData_test();
			//パスワード不正
			$this->app->setUser_test(Sl_user::createModel(array(Sl_user::id=>TestUser::id,Sl_user::password=>"aaaa")));
			$this->getControl()->equals(false,$this->app->registerSessionUser_test());
			$this->app->setupSessionData_test();
			//規制値不足
			$this->app->getAuthDB_test()->startTransaction();
			$user=Sl_user::find($this->app->getAuthDB_test(),Sl_user::createModel(array(Sl_user::id=>TestUser::id)));
			$user[0]->set(Sl_user::restriction,AppConfigRunnable::restrictionLoginValue);
			Sl_user::save($this->app->getDB(ControllerRunnable::basicDbIndex),$user[0]);
			$this->app->setUser_test(Sl_user::createModel(array(Sl_user::id=>TestUser::id,Sl_user::password=>TestUser::testPasswordValue)));
		...
		public function testRun(){
			$this->app->setAction_test("test");
			$this->getControl()->equals($this->app->run(),
					$this->app->getViewPath().Controller::getControllerName().CommonResources::slash."testView".".php");
			$this->app->setAction_test($this->app->analysisAction_test());
			$this->getControl()->equals($this->app->run(),
		...
		#	Sl_user::setupSecurity(($time="5 minute"), $this->app->getUser());
			...
			$this->getControl()->equals(htmlspecialchars(
				...
				htmlspecialchars("<form name=\"test\" action=\"".AppConfig::$appHomeFromBrowserPath.$this->app->getControllerName()."\" method=\"GET\" id='login'>".
					HtmlHelper::text(Sl_user::parseFormName(Model::security), $this->app->getUser()->get(ModelRunnable::security), array(ModelRunnable::formType=>"hidden")),ENT_QUOTES,AppConfig::character));
			$_SESSION=array();
